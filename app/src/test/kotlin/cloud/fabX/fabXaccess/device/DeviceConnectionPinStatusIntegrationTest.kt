...
internal class DeviceConnectionPinStatusIntegrationTest {
    @Test
    fun `given no authentication when getting device pin status then returns http unauthorized`() = withTestApp {
        // given
    ...
    @Test
    fun `given invalid authentication when getting pin connection status then returns http unauthorized`() =
        withTestApp {
            // given
    ...
    @Test
    fun `when getting device pin status then returns status`() = withTestApp {
        // given
    ...
    @Test
    fun `given no authentication when getting device pin status by id then returns http unauthorized`() =
        withTestApp {
            // given
            ...
            // when
            val response = c().get("/api/v1/device/${deviceId}/pin-status")
    ...
    @Test
    fun `given invalid authentication when getting device pin status by id then returns http unauthorized`() =
        withTestApp {
            // given
            val mac = "AABB11CC22DD"
            val deviceId = givenDevice(mac = mac)
    ...
    @Test
    fun `when getting device pin status by id then returns status`() = withTestApp {
        // given
        val mac = "AABB11CC22DD"
        val secret = "49ecad93aac0bdff2915768bd514678f"
    ...
    @Test
    fun `given unknown device when getting device pin status by id then returns http not found`() = withTestApp {
        // given
        val invalidDeviceId = DeviceIdFixture.arbitrary().serialize()

        // when
        val response = c().get("/api/v1/device/${invalidDeviceId}/pin-status") {
            memberAuth()
        }
        ...
        assertThat(response.body<Error>())
            .isError(
                "DevicePinStatusUnknown",
                "Pin status of Device with id DeviceId(value=$invalidDeviceId) not known.",
